...
- `.env` / Netlify 環境變數：`GOOGLE_SHEET_CSV_URL`（視為機密，僅保留在伺服端）、`GOOGLE_MAPS_KEY`、`GOOGLE_MAP_ID`、`HERE_API_KEY`。
...
| **既有測試套件** | 全程維持 106 個 node:test 測試 + `npm run typecheck` 全綠 —— 前端契約在 Phase 4 之前不變 |
...
| **相依項** | Phase 1 通過 |
| **驗收標準** | 對照差異只包含預期中的清理項目；冪等性測試（第二次執行 = 0 筆寫入）；preview 網站視覺上與原本一致；106 個測試 + typecheck 全綠；**改名測試：** 在 Notion 中重新命名一個地點 → 重新匯出 → 其 `id`（Slug）不變，引用它的收藏仍能正確解析；重複 slug 的輸入會被驗證器拒絕 |
...
1. 切換後公開網站的行為與外觀不變（現有 106 個測試套件 + 在正式 URL 上人工抽測）。
