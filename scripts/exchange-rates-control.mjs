...
import { EXCHANGE_KEYS, isValidBreaker, isValidControl, isValidSnapshot } from '../netlify/functions/_shared/exchange-rates-contract.mjs';
import { changeControl } from '../netlify/functions/_shared/exchange-rates-control.mjs';
import { createAdminStore, readEntry } from '../netlify/functions/_shared/exchange-rates-storage.mjs';
...
export function parseArgs(argv) {
  ...
  if (!['status', 'enable', 'disable'].includes(command) || rest.length > 2) {
    throw new Error('Usage: npm run fx:control -- status|enable|disable [--reason code]');
  }
  let reason = 'manual';
  if (rest.length) {
